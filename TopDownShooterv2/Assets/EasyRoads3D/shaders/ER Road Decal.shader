...
Offset[_OffsetFactor] ,[_OffsetUnit]
		...
		Pass
		{
						...
						)
			{
				...
				float2 texCoord21 = packedInput.ase_texcoord5.xy * float2( 1,1 ) + float2( 0,0 );
				float3 unpack5 = UnpackNormalScale( SAMPLE_TEXTURE2D( _BumpMap, sampler_BumpMap, texCoord21 ), _BumpScale );
				...
				surfaceDescription.Albedo = ( tex2DNode1 * _Color ).rgb;
				surfaceDescription.Normal = unpack5;
				surfaceDescription.BentNormal = float3( 0, 0, 1 );
				surfaceDescription.CoatMask = 0;
				surfaceDescription.Metallic = ( tex2DNode2.r * _MainMetallicPower1 );
				...
				surfaceDescription.Smoothness = ( tex2DNode2.a * _MainSmoothnessPower1 );
				surfaceDescription.Occlusion = ( tex2DNode2.g * _OcclusionStrength1 );
		...
		Pass
		{
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				float2 texCoord21 = packedInput.ase_texcoord.xy * float2( 1,1 ) + float2( 0,0 );
				float3 unpack5 = UnpackNormalScale( SAMPLE_TEXTURE2D( _BumpMap, sampler_BumpMap, texCoord21 ), _BumpScale );
				unpack5.z = lerp( 1, unpack5.z, saturate(_BumpScale) );
		...
		Pass
		{
						...
						)
			{
				...
				float2 texCoord21 = packedInput.ase_texcoord7.xy * float2( 1,1 ) + float2( 0,0 );
				float3 unpack5 = UnpackNormalScale( SAMPLE_TEXTURE2D( _BumpMap, sampler_BumpMap, texCoord21 ), _BumpScale );
				unpack5.z = lerp( 1, unpack5.z, saturate(_BumpScale) );
