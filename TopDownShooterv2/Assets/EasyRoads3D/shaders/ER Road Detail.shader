...
Shader "EasyRoads3D/ER Road Detail"
{
	Properties
	{
		...
		_MetallicGlossMap1("Metallic (R) AO (G) Smoothness (A)", 2D) = "gray" {}
	...
	SubShader
	{
		...
		Pass
		{
						...
						)
			{
				...
				float2 texCoord22 = packedInput.ase_texcoord5.zw * float2( 1,1 ) + float2( 0,0 );
				...
				float2 texCoord37 = packedInput.ase_texcoord5.zw * float2( 1,1 ) + float2( 0,0 );
				float3 unpack36 = UnpackNormalScale( SAMPLE_TEXTURE2D( _DetailNormalMap, sampler_DetailNormalMap, texCoord37 ), _DetailNormalMapScale );
				...
				surfaceDescription.Albedo = ( ( ( tex2DNode1 * _Color ) * ( SAMPLE_TEXTURE2D( _DetailAlbedoMap, sampler_DetailAlbedoMap, texCoord22 ) * _Color1 ) ) * float4(2,2,2,2) ).rgb;
				surfaceDescription.Normal = BlendNormal( unpack5 , unpack36 );
		...
		Pass
		{
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				float4 tex2DNode1 = SAMPLE_TEXTURE2D( _MainTex, sampler_MainTex, uv_MainTex );
				float2 texCoord22 = packedInput.ase_texcoord.zw * float2( 1,1 ) + float2( 0,0 );
				
				float2 texCoord26 = packedInput.ase_texcoord.xy * float2( 1,1 ) + float2( 0,0 );
				float3 unpack5 = UnpackNormalScale( SAMPLE_TEXTURE2D( _BumpMap, sampler_BumpMap, texCoord26 ), _BumpScale );
				unpack5.z = lerp( 1, unpack5.z, saturate(_BumpScale) );
				float2 texCoord37 = packedInput.ase_texcoord.zw * float2( 1,1 ) + float2( 0,0 );
				float3 unpack36 = UnpackNormalScale( SAMPLE_TEXTURE2D( _DetailNormalMap, sampler_DetailNormalMap, texCoord37 ), _DetailNormalMapScale );
				unpack36.z = lerp( 1, unpack36.z, saturate(_DetailNormalMapScale) );
		...
		Pass
		{
					...
					)
			{
				...
				float3 unpack5 = UnpackNormalScale( SAMPLE_TEXTURE2D( _BumpMap, sampler_BumpMap, texCoord26 ), _BumpScale );
				unpack5.z = lerp( 1, unpack5.z, saturate(_BumpScale) );
				float2 texCoord37 = packedInput.ase_texcoord3.zw * float2( 1,1 ) + float2( 0,0 );
				float3 unpack36 = UnpackNormalScale( SAMPLE_TEXTURE2D( _DetailNormalMap, sampler_DetailNormalMap, texCoord37 ), _DetailNormalMapScale );
				unpack36.z = lerp( 1, unpack36.z, saturate(_DetailNormalMapScale) );
		...
		Pass
		{
						...
						)
			{
				...
				float4 tex2DNode1 = SAMPLE_TEXTURE2D( _MainTex, sampler_MainTex, uv_MainTex );
				float2 texCoord22 = packedInput.ase_texcoord7.zw * float2( 1,1 ) + float2( 0,0 );
				
				float2 texCoord26 = packedInput.ase_texcoord7.xy * float2( 1,1 ) + float2( 0,0 );
				float3 unpack5 = UnpackNormalScale( SAMPLE_TEXTURE2D( _BumpMap, sampler_BumpMap, texCoord26 ), _BumpScale );
				unpack5.z = lerp( 1, unpack5.z, saturate(_BumpScale) );
				float2 texCoord37 = packedInput.ase_texcoord7.zw * float2( 1,1 ) + float2( 0,0 );
				float3 unpack36 = UnpackNormalScale( SAMPLE_TEXTURE2D( _DetailNormalMap, sampler_DetailNormalMap, texCoord37 ), _DetailNormalMapScale );
				unpack36.z = lerp( 1, unpack36.z, saturate(_DetailNormalMapScale) );
